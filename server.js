...
function loadGeocodeCache() {
  if (!fs.existsSync(GEO_CACHE_FILE)) return new Map();

  const raw = fs.readFileSync(GEO_CACHE_FILE, "utf-8");
...
function saveGeocodeCache() {
  ...
  fs.writeFileSync(GEO_CACHE_FILE, JSON.stringify(data, null, 2));
}
...
async function normalizeQ8(raw) {
  ...
  for (const station of list) {
    ...
    const coords = geocodeCache.get(address) || { lat: null, lng: null };
